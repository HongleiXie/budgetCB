...
class LinUCBTest(unittest.TestCase):
    def setUp(self):
        np.random.seed(2020)
        self.config = {'narms': 3, 'ndims': 3, 'alpha': 0.1, 'T': 10, 'B': 3, 'dummy_arm': 0}
    ...
    def test_replay_payoffs(self):
        evaluator = ReplayEvaluator(policy=self.policy, arms=self.action, rewards=self.reward, contexts=self.state)
        actual_output = evaluator.evaluate()
        expected_output = {'avg_payoff': 0.5719}
        details = evaluator.get_details()
        self.assertAlmostEqual(actual_output['avg_payoff'], expected_output['avg_payoff'], places=3)
        self.assertEqual(details['match_num'], 6)
        self.assertEqual(details['match_arms'], [0] * 6)
        self.assertEqual(['match_num', 'payoff_records', 'match_arms'], [x for x in details.keys()])